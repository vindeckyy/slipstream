...
function buildSamples(n: number): StatsSample[] {
	...
	for (let i = 0; i < n; i++) {
		...
		out.push({
			...
			mbps: 920 + wobble * 55,
			bitrate_kbps: 150_000,
			capture_age_over_limit: i % 20 === 0,
			capture_age_us: 1_200 + Math.round(Math.abs(wobble) * 700),
			capture_backend: "pipewire",
			capture_buffers_drained: Math.floor(i / 13),
			capture_frames_overwritten: Math.floor(i / 24),
			capture_frames_published: (i + 1) * 240,
			capture_width: 5120,
			capture_height: 1440,
			capture_modifier: 0,
