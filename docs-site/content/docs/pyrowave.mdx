...
- **4:4:4 (full chroma).** With your client's **4:4:4** setting on, the session encodes chroma
  ...
  luma, so it is less than the 2x the extra samples imply). Available on Linux and Windows
  hosts.
...
- The two combine: a 4:4:4 **and** HDR session applies both factors (~1.6 x 1.15  approx  1.9x the
