...
|---|---|---|---|
| `ci.yml` | push to `main`, PRs | `ubuntu-latest` | Rust workspace (fmt · clippy `-D warnings` · build · test · C-ABI harness · generated-header drift) inside the `slipstream-rust-ci` image; `web/` and `docs-site/` build + typecheck in `oven/bun:1` |
...
- **`macos-arm64`** — `home-mac-mini-1` (M-series, macOS 26), a **host-mode**
  ...
  `runner.log`), kept alive by the `io.github.act_runner` LaunchAgent. Needs full **Xcode**
...
- **Mac runner offline** — `ssh <mac> tail -50 '~/ci/act-runner/runner.log'`; restart with
  `launchctl kickstart -k gui/$(id -u)/io.github.act_runner`. After a reboot with nobody
