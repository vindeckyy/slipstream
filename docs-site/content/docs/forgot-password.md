...
password. New Linux and SteamOS installs ask you to choose it in the browser. It lives on the
**host**, so if you can't get past the login screen, recover or change it on the host machine itself.
...
the exact edit-and-restart steps for each platform above, and it's the one place that procedure is
kept up to date.
