---
title: Plugins
description: First-party plugins, sync your ROM collection into the game library, or hand a real USB device on the couch to the host, and how to install them.
---
...
A bare name resolves to the first-party package, `rom-manager` installs
`@slipstream/plugin-rom-manager`, always from Slipstream's own package registry. Any other name
(`slipstream-plugin-*`, a foreign
...
(`systemctl --user restart slipstream-scripting`).
...
wrote them, `~/.config/slipstream/plugin-state/<plugin>/`, so re-installing later picks your settings
back up. Delete that directory yourself if you want it gone.
...
`plugin-state` is where a plugin reads and writes its own files.
...
Servers exist for Linux and Android couches. **There is no VirtualHere server for iOS**, so iPhones
cannot pass devices through, nothing on the Slipstream side can change that.
...
**`slipstream-host: command not found`**, the host package installs it to `/usr/bin/slipstream-host`.
...
On SteamOS, re-run `scripts/steamdeck/install.sh` (or `scripts/steamdeck/update.sh`).
...
`slipstream-plugin-rom-manager`, which is the reference implementation, or either of the plugins
above.
