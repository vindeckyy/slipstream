...
export type InputConfig = { readonly "gamepad"?: string | null, readonly "gamescope_grab_cursor"?: boolean, readonly "pen"?: boolean }
export const InputConfig = Schema.Struct({ "gamepad": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Gamepad backend preference (`SLIPSTREAM_GAMEPAD`)." })), "gamescope_grab_cursor": Schema.optionalKey(Schema.Boolean.annotate({ "description": "Gamescope: grab cursor into the nested session.\nDefault off (matches runtime `SLIPSTREAM_GAMESCOPE_GRAB_CURSOR`)." })), "pen": Schema.optionalKey(Schema.Boolean.annotate({ "description": "Advertise full-fidelity pen/stylus input (`SLIPSTREAM_PEN`). Default on." })) }).annotate({ "identifier": "InputConfig" })
