...
export type ApiFieldError = { readonly "field": string, readonly "message": string }
export const ApiFieldError = Schema.Struct({ "field": Schema.String, "message": Schema.String }).annotate({ "description": "One field-level validation problem, keyed by the dotted setting path\n(e.g. `audio_video.max_fps`) so the console can anchor the error to the control.", "identifier": "ApiFieldError" })
...
export const GameMeta = Schema.Struct({ "description": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Short blurb for a details pane." })), "developer": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])), "genres": Schema.optionalKey(Schema.Array(Schema.String).annotate({ "description": "Genre taxonomy from the metadata source, such as `\"RPG\"` or `\"Platformer\"`." })), "platform": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "The system the title runs on, such as `\"PS2\"`, `\"Xbox 360\"`, `\"SNES\"`, or `\"PC\"`. Installed-store\nscanners stamp `\"PC\"`; `GET /library?platform=` filters on it (case-insensitive)." })), "players": Schema.optionalKey(Schema.Union([Schema.Number.check(Schema.isInt().annotate({ "expected": "an integer" })).check(Schema.isGreaterThanOrEqualTo(0).annotate({ "expected": "a value greater than or equal to 0" })), Schema.Null]).annotate({ "description": "Maximum simultaneous (local) players.", "format": "int32" })), "publisher": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])), "region": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Emulation release region, such as `\"NTSC-U\"`, `\"PAL\"`, or `\"NTSC-J\"`." })), "release_year": Schema.optionalKey(Schema.Union([Schema.Number.check(Schema.isInt().annotate({ "expected": "an integer" })).check(Schema.isGreaterThanOrEqualTo(0).annotate({ "expected": "a value greater than or equal to 0" })), Schema.Null]).annotate({ "description": "Year of first release, the granularity metadata sources reliably agree on.", "format": "int32" })), "tags": Schema.optionalKey(Schema.Array(Schema.String).annotate({ "description": "Free-form organizational labels, such as `\"co-op\"`, `\"kids\"`, or `\"finished\"`." })) }).annotate({ "description": "Descriptive metadata for a title. This covers everything a richer library UI (details pane, platform\nfilter, couch-pick badges) renders beyond the poster. Every field is optional and defaults to\nabsent, so pre-metadata catalogs, providers, and clients keep working unchanged. The struct is\nflattened into [`GameEntry`] and the custom-store shapes: one definition, a flat\nwire shape everywhere.\n\nValues are free-form display strings, not enums. Emulation sources such as RomM, EmuDeck, and Lutris\neach have their own vocabulary and the host has no business normalizing it.", "identifier": "GameMeta" })
...
export type ApiError = { readonly "error": string, readonly "fields"?: ReadonlyArray<ApiFieldError> | null }
export const ApiError = Schema.Struct({ "error": Schema.String, "fields": Schema.optionalKey(Schema.Union([Schema.Array(ApiFieldError), Schema.Null]).annotate({ "description": "Field-keyed validation issues, present only on 400 validation failures." })) }).annotate({ "description": "Error envelope for every non-2xx response.", "identifier": "ApiError" })
...
export type GetCustomGame409 = ApiError
export const GetCustomGame409 = ApiError
...
export const make = (
  ...
  return {
    ...
    withResponse(options?.config)(HttpClientResponse.matchStatus({
      ...
      "409": decodeError("GetCustomGame409", GetCustomGame409),
      orElse: unexpectedStatus
    }))
  ),
...
*/
readonly "getCustomGame": <Config extends OperationConfig>(id: string, options: { readonly config?: Config | undefined } | undefined) => Effect.Effect<WithOptionalResponse<typeof GetCustomGame200.Type, Config>, HttpClientError.HttpClientError | SchemaError | SlipstreamError<"GetCustomGame401", typeof GetCustomGame401.Type> | SlipstreamError<"GetCustomGame404", typeof GetCustomGame404.Type> | SlipstreamError<"GetCustomGame409", typeof GetCustomGame409.Type>>
  /**
