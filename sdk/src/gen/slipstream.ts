...
export const Objects_ = Schema.Struct({ "mode": Schema.Literal("off") }).annotate({ "description": "Tear the display down at session end (today's default on Linux backends)." })
...
* patch). Applies from the next event — no restart. Hook commands run as the host user (interactive user session): treat this configuration as operator-privileged.
*/
...
* everywhere; Lutris + Heroic on Linux), so the console renders a toggle only for scanners that can do anything here. Scanners default to enabled;
...
readonly "supportBundleDelete": <Config extends OperationConfig>(id: string, options: { readonly config?: Config | undefined } | undefined) => Effect.Effect<WithOptionalResponse<void, Config>, HttpClientError.HttpClientError | SchemaError | SlipstreamError<"SupportBundleDelete401", typeof SupportBundleDelete401.Type> | SlipstreamError<"SupportBundleDelete404", typeof SupportBundleDelete404.Type>>
  /**
* Starts the one-click apply for a verified package update. The request carries no version or URL — the host installs exactly what its verified manifest
