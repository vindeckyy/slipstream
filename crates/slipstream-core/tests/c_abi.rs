...
//! against the freshly built `libslipstream_core.a`, and asserts it round-trips frames
...
fn ensure_staticlib(profile_dir: &Path) -> PathBuf {
    let staticlib = profile_dir.join("libslipstream_core.a");
    if !staticlib.exists() {
...
fn c_abi_harness_round_trips() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR")); // crates/slipstream-core
    ...
    assert!(
        staticlib.exists(),
        "staticlib not found at {} (run `cargo build -p slipstream-core`)",
        staticlib.display()
    );
    assert!(
        include.join("slipstream_core.h").exists(),
        "generated header missing; build slipstream-core to regenerate it"
    );

    let cc = std::env::var("CC").unwrap_or_else(|_| "cc".into());
    let out = profile_dir.join("slipstream_c_harness");
