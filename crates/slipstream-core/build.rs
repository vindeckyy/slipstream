//! Generate the C header (`include/slipstream_core.h`) from the `extern "C"` surface.
//!
...
fn main() {
    ...
    // Workspace-level include/ dir: crates/slipstream-core/ -> ../../include/
    let out = PathBuf::from(&crate_dir)
        .join("..")
        .join("..")
        .join("include")
        .join("slipstream_core.h");
