...
captures it on the GPU, encodes with hardware NVENC/VAAPI, and sends it out over a
...
Runs on **Linux**. The shared protocol,
...
- **GPU zero-copy capture → encode.** dmabuf → CUDA/Vulkan → NVENC on Linux → GPU encode.
  Encoders auto-select by GPU vendor: **NVENC** (NVIDIA), **VAAPI** (AMD/Intel), or software H.264
  as a floor. HDR/10-bit and HEVC 4:4:4 supported.
- **Input injection.** Mouse/keyboard (libei / gamescope EIS / wlr) and virtual
...
[`packaging/`](../../packaging/README.md) (build local debs/rpms, COPR/bootc, Arch, and sysext) and
the Linux guide under
...
src/
  ...
  capture/ · capture.rs    screen/dmabuf capture
  encode/ · encode.rs      per-GPU encoders (nvenc · vaapi · sw)
  ...
  linux/                platform-confined backends
```
