...
//! encoded access units also fed through a `slipstream_core` host→client `Session` over an
...
use slipstream_core::{Config, Role, Session};
use std::fs::File;
...
pub struct Options {
    ...
    pub out: PathBuf,
    /// Also round-trip every AU through a `slipstream_core` host→client loopback and verify.
    pub loopback: bool,
}
...
pub fn run(opts: Options) -> Result<()> {
    let mut capturer: Box<dyn Capturer> = match opts.source {
        ...
        Source::Portal => {
            ...
            let want_hdr = std::env::var("SLIPSTREAM_SPIKE_HDR").as_deref() == Ok("1");
            tracing::info!(
                want_hdr,
        ...
        Source::KwinVirtual => {
            ...
            tracing::info!(
                ...
                height = opts.height,
                ?compositor,
                "spike source: virtual output (SLIPSTREAM_COMPOSITOR)"
            );
    ...
    let mut lb = if opts.loopback {
        Some(Loopback::new().context("build slipstream-core loopback")?)
    } else {
        None
    };
    ...
    if let Some(lb) = lb {
        ...
        if lb.mismatches > 0 || lb.recovered != lb.submitted {
            return Err(anyhow!(
                "slipstream-core loopback verification FAILED: {} mismatches, {}/{} AUs recovered",
                lb.mismatches,
                lb.recovered,
                lb.submitted
            ));
...
/// A host↔client `slipstream_core` pair over a lossless in-process loopback. Each encoded AU is
...
impl Loopback {
    fn new() -> Result<Loopback> {
        let (host_tx, client_tx) = slipstream_core::transport::loopback_pair(0, 0);
    ...
    fn submit(&mut self, au: &EncodedFrame) -> Result<()> {
        ...
        loop {
            match self.client.poll_frame() {
                ...
                Err(slipstream_core::SlipstreamError::NoFrame) => break,
    ...
    fn report(&self) {
        tracing::info!(
            ...
            bytes = self.bytes,
            "slipstream-core loopback: AUs FEC-packetized → sent → reassembled & verified"
        );
    }
}
