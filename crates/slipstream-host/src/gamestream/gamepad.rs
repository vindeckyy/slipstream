...
// ([`slipstream_core::input::MAX_PADS`]) are shared vocabulary between this Moonlight decode path and
...
// The `BTN_*` button bitmask and axis ids live in `slipstream_core::input::gamepad` — GameStream's
// `buttonFlags | buttonFlags2 << 16` layout (Limelight.h) is bit-identical to slipstream's native
...
mod tests {
    use super::*;
    use slipstream_core::input::gamepad::{BTN_A, BTN_RB};
