<#
.SYNOPSIS
  Build + sign the ss-vdisplay UMDF IddCx virtual-display driver FROM SOURCE, in CI, and stage it for the
  host installer. This REPLACES the old vendored-prebuilt-binary model (packaging/windows/ss-vdisplay/) -
...
.DESCRIPTION
  ...
  catalog -> export the public .cer. Output (-Out): ss_vdisplay.{dll,inf,cat} + slipstream-driver.cer.
...
.EXAMPLE
  pwsh -File build-ss-vdisplay.ps1 -Out C:\t\pfvd -DriverVer 9.9.0626.1612
#>
[CmdletBinding()]
param(
    ...
    [switch]$SkipBuild                                    # reuse an existing target\...\release\ss_vdisplay.dll
)
...
$inx = Join-Path $DriversDir 'ss-vdisplay\ss_vdisplay.inx'
$clear = Join-Path $PSScriptRoot 'clear-force-integrity.ps1'
if (-not (Test-Path $inx)) { throw "no ss_vdisplay.inx under $DriversDir" }
...
$dll = Join-Path $drvTarget 'x86_64-pc-windows-msvc\release\ss_vdisplay.dll'

# --- 1. build (release) -----------------------------------------------------------------------
if (-not $SkipBuild) {
    Write-Host "==> cargo build --release (ss-vdisplay) in $DriversDir (default target -> $drvTarget)"
    ...
    if ($rc -ne 0) { throw "ss-vdisplay cargo build failed ($rc)" }
}
...
$sDll = Join-Path $Out 'ss_vdisplay.dll'
$sInf = Join-Path $Out 'ss_vdisplay.inf'
$sCat = Join-Path $Out 'ss_vdisplay.cat'
...
$cat = $null
try { $cat = Test-FileCatalog -CatalogFilePath $sCat -Path $Out -FilesToSkip 'ss_vdisplay.cat', 'slipstream-driver.cer' -Detailed }
...
if ($cat) {
    ...
    foreach ($need in @('ss_vdisplay.inf', 'ss_vdisplay.dll')) {
        if (-not ($covered | Where-Object { $_ -like "*$need" })) {
            throw "catalog coverage guard: $need is NOT in $sCat (stampinf/Inf2Cat ordering bug?)"
        }
    }
    Write-Host "    catalog covers ss_vdisplay.inf + ss_vdisplay.dll (status=$($cat.Status))"
}

Write-Host "==> built + signed ss-vdisplay  DriverVer=$DriverVer  ->  $Out"
