...
//! shared `ss-driver-proto` ABI crate (no_std + bytemuck) across the workspace boundary.
...
/// Force `ss-driver-proto` to actually link into the driver build graph (validates the cross-workspace
/// path-dep + that the no_std bytemuck ABI crate compiles for a UMDF cdylib). `#[used]` keeps it.
#[used]
static PROTO_GUID_LO: u64 = ss_driver_proto::PF_VDISPLAY_INTERFACE_GUID_U128 as u64;
