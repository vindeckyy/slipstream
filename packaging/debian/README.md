...
```sh
VERSION=0.24.0 bash packaging/debian/build-deb.sh   # -> dist/slipstream-host_0.24.0_amd64.deb
```
...
docker run --rm -v "$PWD:/src" -w /src ss-noble \
  bash -lc 'VERSION=0.24.0 BUNDLE_FFMPEG=1 bash packaging/debian/build-deb.sh'
```
...
docker run --rm -v "$PWD:/w" -w /w ss-arm64cross \
  bash -lc 'VERSION=0.24.0 ARCH=arm64 TARGET=aarch64-unknown-linux-gnu \
