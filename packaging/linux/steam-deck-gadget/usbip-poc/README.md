...
captured descriptors + feature contract from `crates/ss-inject/src/input/linux/steam_gadget.rs`.
...
The prototype still depends on `rusb`/libusb and the external `usbip` CLI. The `usbip` crate API
uses a custom `UsbInterfaceHandler`:
