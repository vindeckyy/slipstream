#!/usr/bin/env bash
# Wrap a built slipstream-host pacman package into a systemd-sysext image — the update-survivable
...
#
# Usage:  bash build-sysext.sh <slipstream-host-*.pkg.tar.zst>
# Output: slipstream-host.raw
set -euo pipefail

PKG="${1:?usage: build-sysext.sh <slipstream-host-*.pkg.tar.zst>}"
