...
# The console's plugin store and `slipstream-host plugins …` shell out to the scripting runner —
...
bun build src/runner-cli.ts --target=bun --outfile \"\$HOME/.local/share/slipstream-scripting/runner-cli.js\"
...
install -m0755 \"\$(command -v bun)\" \"\$HOME/.local/lib/slipstream-scripting/bun\"
"
...
#!/bin/sh
# Generated by scripts/steamdeck/install.sh — user-scoped slipstream-scripting (the .deb's /usr/bin
...
ok "plugin runner: ~/.local/bin/slipstream-scripting"
...
sed 's|^ExecStart=.*|ExecStart=%h/.local/bin/slipstream-scripting|' \
    "$SRC/scripts/slipstream-scripting.service" > "$UNITS/slipstream-scripting.service"
ok "slipstream-scripting.service (opt-in: systemctl --user enable --now slipstream-scripting)"

# Post-OS-update self-heal: SteamOS A/B updates can bump library sonames the host binary links
# (FFmpeg/PipeWire/libva) — this oneshot probes the binary with ldd before slipstream-host starts
# and re-runs update.sh only when it actually stopped loading. Milliseconds on a normal boot.
cat > "$UNITS/slipstream-rebuild-check.service" <<EOF
...
ok "slipstream-rebuild-check.service (auto-rebuild after SteamOS updates)"
