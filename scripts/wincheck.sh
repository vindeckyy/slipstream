...
#
# The whole Windows capture stack uses exactly TWO items from slipstream-core — `quic::HdrMeta` and
# `Mode`, both plain data (re-verify with:
# `grep -rho 'slipstream_core::[A-Za-z_:]*' crates/pf-capture/src crates/pf-frame/src crates/pf-win-display/src | sort -u`).
...
MEMBERS_RE='slipstream-core|pf-frame|pf-win-display|pf-capture'
...
# Mirror the real [workspace.package] so the members' `version.workspace = true` resolves.
{
  ...
  echo '[workspace]'
  echo 'resolver = "2"'
  echo 'members = ["slipstream-core", "pf-frame", "pf-win-display", "pf-capture"]'
  echo
