...
# `grep -rho 'slipstream_core::[A-Za-z_:]*' crates/pf-{capture,frame,win-display,vdisplay}/src | sort -u`).
...
MEMBERS_RE='slipstream-core|pf-encode|pf-frame|pf-win-display|pf-capture|pf-vdisplay'
...
# Mirror the real [workspace.package] so the members' `version.workspace = true` resolves.
{
  ...
  echo '[workspace]'
  echo 'resolver = "2"'
  echo 'members = ["slipstream-core", "pf-encode", "pf-frame", "pf-win-display", "pf-capture", "pf-vdisplay"]'
  echo
...
/// Mirrors `slipstream_core::CompositorPref` (config.rs). pf-vdisplay matches on every variant and
