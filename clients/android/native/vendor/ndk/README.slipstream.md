...
`AMediaCodec_setOnFrameRenderedCallback` through `ndk-sys`. The wrapper does not expose the
render-timestamp callback used by the HUD's `display` stage. Search for `slipstream vendored patch`
to find the change.
