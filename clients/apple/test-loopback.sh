#!/usr/bin/env bash
# Loopback integration: a real slipstream/1 host (synthetic source — pure protocol, runs fine on
...
cd clients/apple
SLIPSTREAM_LOOPBACK_PORT="$PORT" swift test --filter LoopbackIntegrationTests
