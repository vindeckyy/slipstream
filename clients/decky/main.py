...
# ship that layout (controller_config/slipstream.vdf, built on Steam's gamepad-fps template) and
# point our shortcuts at it, EmuDeck-style: drop it in controller_base/templates/ (so it is also
# a selectable "Slipstream" template) AND set each account's configset entry for our shortcut's
# game key to that template. Steam keys non-Steam games by their LOWERCASE NAME (verified on the
# Deck: our "Slipstream" shortcut → the "slipstream" configset key), so both our shortcuts (same
# name) share one entry. controller_neptune = the Deck's built-in controller type.
CONTROLLER_TEMPLATE = "slipstream.vdf"
...
class Plugin:
    ...
    async def apply_controller_config(self, name: str = "Slipstream") -> dict:
        ...
        #    configset write on exit): controller_base/templates/slipstream.vdf.
        try:
